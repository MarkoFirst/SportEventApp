...
class LoginVC: UIViewController {
    ...
    private func registrationCD() {
        let fetchRequest: NSFetchRequest<RegistrationCoreData> = RegistrationCoreData.fetchRequest()
        let context = CoreDataService().persistentContainer.viewContext
        let users = try? context.fetch(fetchRequest)
        switch(users?.first(where: { $0.email == enteredEmail.text }) != nil, users?.first(where: { $0.password == enteredPassword.text }) != nil) {
        case(true, true):
        ...
        case(false, true):
            let alert = UIAlertController(title: "Oops!", message: "Incorrect email!", preferredStyle: .actionSheet)
            let action = UIAlertAction(title: "Cancel", style: .cancel)
            alert.addAction(action)
            present(alert, animated: true)
